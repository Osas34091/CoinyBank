          ...
          <div className="flex items-center mb-8">
            ...
            <div>
              ...
              <p className="text-slate-600">
                {locale === 'en' ? 'Manage your active loans.' : 'Mantén al día tus créditos.'}
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-4">
            {loans.length === 0 ? (
              <p className="text-slate-500 text-center py-8">
                {locale === 'en' ? 'No active loans.' : 'No tienes préstamos activos.'}
              </p>
            ) : (
              loans.map((loan: any) => (
                <div key={loan.id} className="flex flex-col md:flex-row justify-between items-start md:items-center p-4 border border-slate-100 rounded-2xl bg-slate-50 gap-4 md:gap-0">
                  <div>
                    ...
                    <p className="text-xs font-bold text-slate-400 mt-1 uppercase">{loan.status}</p>
                  </div>
